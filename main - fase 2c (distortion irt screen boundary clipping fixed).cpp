...
private:
    ...
    void drawWall( int x1, int x2, int b1, int b2, int t1, int t2, int s, int w, int frontBack ) {
        ...
        float ht = 0.0f, ht_step = float( Textures[wt].w ) / float( x2 - x1 );
        ...
        for (int x = x1; x < x2; x++) {
            ...
            float vt = 0.0f, vt_step = float( Textures[wt].h ) / float( y2 - y1 );
            ...
            if (frontBack == 0) {
                ...
                for (int y = y1; y < y2; y++) {                  // normal wall
                    ...
                    int pixel = (int)(Textures[wt].h - 1 - vt) * 3 * Textures[wt].w + (int)ht * 3;
