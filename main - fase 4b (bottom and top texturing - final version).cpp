...
#define res         4                 // 1 = 160x120, 2 = 320x240, 4 = 640x480
...
private:
    ...
    void drawWall( int x1, int x2, int b1, int b2, int t1, int t2, int s, int w, int frontBack ) {
        ...
        for (int x = x1; x < x2; x++) {
            ...
            if (frontBack == 1) {
                ...
                float fov = 200.0f;    // field of view

                int x2 = x - xo;       // x - x offset
                int wo;                // wall offset
                float tile = S[s].ss * 7;    // "it looks best multiplied by 7"

                if (S[s].surface == 1) { y2 = S[s].surf[x]; wo = S[s].z1; }    // bottom surface
                if (S[s].surface == 2) { y1 = S[s].surf[x]; wo = S[s].z2; }    // top    surface


                float lookUpDown = -P.l *  6.2f;   // 3DSage: 6.2f value is found by trial and error
                if (lookUpDown > SH) { lookUpDown = SH; }
                float moveUpDown =  (float)(P.z - wo) / (float) yo; if (moveUpDown == 0.0f) { moveUpDown = 0.0001f; }
                // make floor visible from underneath
                int ys = y1 - yo, ye = y2 - yo;    // y start and y end

                for (int y = ys; y < ye; y++) {

                    float  z = y + lookUpDown; if (z == 0) { z = 0.0001f; }         // prevent division by 0
                    float fx = x2  / z * moveUpDown * tile;                                // world floor x
                    float fy = fov / z * moveUpDown * tile;                                // world floor y
                    // offset the rotated points by the players position
                    // the division by 30.0f is to help "slow it down"
                    float rx = fx * M.sin[P.a] - fy * M.cos[P.a] + (P.y / (60.0f * res) * tile);   // rotated texture x - 3DSage: 60.0f value is trial and error
                    float ry = fx * M.cos[P.a] + fy * M.sin[P.a] - (P.x / (60.0f * res) * tile);   // rotated texture x

                    if (rx < 0) { rx = -rx + 1.0f; }                                // prevent negative values ...
                    if (ry < 0) { ry = -ry + 1.0f; }                                // ... and add 1 for accuracy
                    // draw textures
                    int st = S[s].st;   // surface texture
                    // since we save 3 values per pixel, we need to times by 3 to skip to the next pixel
                    int pixel = (int)(Textures[st].h - 1 - ((int)ry % Textures[st].h)) * 3 * Textures[st].w +
    ...
    void floors() {
        ...
        float fov = 200.0f;
        float lookUpDown = -P.l *  4.0f; if (lookUpDown > SH) { lookUpDown = SH; }
        float moveUpDown =  P.z / 16.0f; if (moveUpDown == 0) { moveUpDown = 0.0001f; }
        // make floor visible from underneath
        ...
        for (int y = ys; y < ye; y++) {
            for (int x = -xo; x < xo; x++) {
                ...
                // offset the rotated points by the players position
                // the division by 30.0f is to help "slow it down"
                float rx = fx * M.sin[P.a] - fy * M.cos[P.a] + (P.y / 30.0f);   // rotated texture x
                float ry = fx * M.cos[P.a] + fy * M.sin[P.a] - (P.x / 30.0f);   // rotated texture x

                if (rx < 0) { rx = -rx + 1.0f; }                                // prevent negative values ...
                if (ry < 0) { ry = -ry + 1.0f; }                                // ... and add 1 for accuracy

                // prevent unwanted poor pixelation
                if (rx <= 0 || ry <= 0 || rx >= 5 || ry >= 5) { continue; }     // determines scope of the world to render
